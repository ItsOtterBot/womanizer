...
/// - `wake`: clone of the DspWakeHandle bound to this worker thread; the capture-pump
///   thread holds the other clone and calls `wake.wake()`.
...
pub fn spawn(
    ...
    let dsp_thread = spawn_dsp_worker(
        in_rx,
        vo_tx,
        mo_tx,
        hot,
